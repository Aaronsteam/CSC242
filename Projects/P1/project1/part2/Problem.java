...
public class Problem {
    ...
    private void humanMove() {
        ...
        do {
            if(board != 0) System.err.println("Enter the board: " + "(" + board + ")");
            else System.err.println("Enter the board: ");
            boardChoice = scanner.nextInt();
            if(board != 0 && boardChoice != board) {
                while(boardChoice != board) {
                    System.err.println("Please enter the correct board" + "(" + board + ")");
                    boardChoice = scanner.nextInt();
                }
            }
            System.err.println("Enter the position: ");
            position = scanner.nextInt();
        } while (!activeState.spotEmpty(activeState.getBoard()[boardChoice-1][position-1]));

        activeState = result(activeState, new Action(boardChoice, position));
    ...
    private void aiMove() {
        ...
        Action decision = alphaBetaDecision(activeState);
        long lEndTime = System.nanoTime();
        System.err.println("Decision took " + (lEndTime - lStartTime) + " milliseconds");
    ...
    private Action alphaBetaDecision(State s){
        int max = Integer.MIN_VALUE;
        int minVal;
        Action aDecision = new Action(0,0);
        for(Action a: applicableActions(s)) {
            minVal = minVal(result(s,a), Integer.MIN_VALUE, Integer.MAX_VALUE);
            if(minVal > max) {
                max = minVal;
                aDecision = a;
            }
    ...
    private int maxVal(State s, int a, int b) {
        if(cutOff(s)) return eval(s);

        int v = Integer.MIN_VALUE;

        for(Action action: applicableActions(s)) {
            v = Math.max(v, minVal(result(s,action), a, b));
            if(v >= b) return v;
            a = Math.max(a, v);
        }
    ...
    private int minVal(State s, int a, int b) {
        if(cutOff(s)) return eval(s);

        int v = Integer.MAX_VALUE;

        for(Action action: applicableActions(s)) {
            v = Math.min(v, maxVal(result(s,action), a, b));
            if(v <= a) return v;
            b = Math.min(b, v);
        }
